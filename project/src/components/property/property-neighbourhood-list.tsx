...
export default function PropertyNeighbourhoodList(
  {hostels, onEnterFunction, hostelInProperty}:PropertyNeighbourhoodListOptions,
):JSX.Element {
  const hostelsOnNeighbourhood = hostels.filter((hostel) => hostel.id !== hostelInProperty.id);
  const arrayPoints = hostelsOnNeighbourhood.map((hostel) => <PointItem hostel = {hostel} key = {hostel.id} onEnterFunction = {onEnterFunction}/>);
  return (
